...
> **只想把地图裁干净，不需要地理坐标？**
> 那更简单 —— 文件名乱、没有接图表、不知道经纬度**都没关系**，
> 只要跑 `python scripts/01_crop.py` 就够了。
> 见 **[docs/只裁切模式.md](docs/只裁切模式.md)**。
...
**先看这张图** —— 你是哪种情况，就走哪条路，不用读完全文。

```mermaid
flowchart TD
    A[一批地图扫描件] --> B{"需要地理坐标吗<br/>要跟别的图层叠着看吗"}
    B -->|"不要，只想裁干净"| C["只裁切模式"]
    B -->|"要，能进 GIS"| D{"文件名里有图号吗<br/>或能查到接图表"}
    C --> C1["双击 自定义尺寸裁切.bat"]
    C --> C2["或 python scripts/01_crop.py -s 3000x2000"]
    D -->|"没有"| P{"图是一套连续的吗"}
    D -->|"有"| F["核对图号换算规则"]
    P -->|"是，想拼成整幅"| Q["拼图工作台<br/>拖拽确定位置关系"]
    P -->|"否，各不相干"| C
    Q --> Q2["assemble_by_layout.py<br/>拼成整幅"]
    F --> G["单幅试跑确认"]
    G --> H["双击 一键运行.bat"]
    H --> I{"结果有问题吗"}
    I -->|"某幅被切了内容"| J["per_sheet 放宽阈值<br/>只重跑那一幅"]
    I -->|"其他报错"| K["查 新手从这里开始.md"]
```
...
| 你想要的 | 走哪条 | 需要什么 | 花多久 |
|---|---|---|---|
| **只是把图裁干净** | `自定义尺寸裁切.bat` | 只有扫描件 | 几分钟 |
| **要能进 GIS 叠图分析** | `一键运行.bat` | 扫描件 + 图号（文件名） | 几分钟 + 前期核对 |
| **没有图号，但图是连续的一套** | `拼图工作台.bat` → 拖拽 → 点「一键生成拼图」 | 只有扫描件 | 拖几十分钟（看幅数） |
| **单幅返工** | `01_crop.py 文件名` | 已跑过一次 | 几秒 |

> **"图是一套连续的"怎么判断？** 看几幅缩略图：相邻两幅的边缘能接上
> （同一条河、同一条路继续延伸）就是一套。
> 见 **[docs/拼图工作台.md](docs/拼图工作台.md)**。

### 三种运行方式

| 方式 | 适合谁 | 怎么做 |
|---|---|---|
| **双击 `.bat`** | 不想碰命令行 | `一键运行.bat`（全流程）<br>`自定义尺寸裁切.bat`（只裁切，可自填尺寸）<br>`拼图工作台.bat`（没图号时拼位置）<br>`拼接成果.bat`（拼成整图）<br>`生成示例数据.bat`（先试效果） |
| **命令行** | 有一点基础 | `python scripts/01_crop.py -s 3000x2000` |
| **交给 Agent** | 想省事 | 把仓库路径给 AI，让它读文档、改配置、跑命令 |

> **不确定走哪条？** 先用 `生成示例数据.bat` + `一键运行.bat` 跑一遍完整流程，
> 看看产物长什么样，再决定自己的数据要不要配准。
...
它会自动检查 Python 环境、安装缺失的依赖，然后**问你三件事**，最后打开成果文件夹。

### 它会问你什么

**这三件事没有"对错"，取决于你这批图要拿来干什么** —— 所以不替你决定：
...
[3/3] 这批图的文件名里有标准图号吗？
   1 = 有   形如 2108-瑪瑙觀.jpg（前两位列号、后两位行号）  【默认】
   2 = 没有 我要做地理配准，但文件名不是这个格式
   3 = 没有 我只要裁切，不需要经纬度
```

**每题直接回车 = 用括号里的默认值**，不用一路敲。

**第 3 题最关键**：选 3 就**跳过地理配准**（因为算不出经纬度），
只做裁切 —— 以前这一步会直接报错退出，白跑一遍。
...
```cmd
一键运行.bat /color gray /size original /geo 0
```

给了任意一个参数就不再问，**没给的用默认值**。

| 参数 | 取值 |
|---|---|
| `/color` | `auto` / `gray` / `color` |
| `/size` | `4500x3500` / `original` / 其他尺寸如 `3000x2000` |
| `/geo` | `1` = 做配准，`0` = 只裁切 |
...
> 完整的命名规则说明（如何适配其他编号体系、如何用图集接图表校验、
> 怎么发现缺幅与错位）见 **[docs/命名规则与接图表.md](docs/命名规则与接图表.md)**。

### 不确定输出尺寸该填多少？先探测

`crop.out_size` 该填多少原来只能靠猜 —— 填小了浪费扫描精度，填大了白占空间，
填错了还会把地图拉变形，而画面上根本看不出来。现在可以直接测：

```bash
python scripts/tools/probe_resolution.py
```

它抽样跑一遍「纠斜 → 找图廓 → 找内容边界」，统计每幅**裁到内容后的真实像素尺寸**，
给出几档可选尺寸并说明各自代价：

```
--- 内容区尺寸（裁到内容边界后、缩放前）---
  宽度   最小  4435   中位  4540   最大  5794
  高度   最小  3231   中位  3512   最大  4399
  宽高比 最小 1.243   中位 1.296   最大 2.385

--- 建议（按你的用途选）---
  ① [4500, 3500]   轻微缩放，尺寸统一 —— 用于经纬度配准时选这个
  ② [5794, 4399]   保留最大图幅的原始分辨率
  ③ [3020, 2330]   约 2/3 尺寸，适合屏幕浏览
  ④ original        不缩放，各幅保持原始像素
```

**还会自动提示两类问题：**

- **宽高比异常**（正常地图约 0.9~1.9）—— 多半是接图表、封面混进了 `input/`，
  提示你把它加进 `crop.exclude`
- **扫描分辨率不一致** —— 有图幅明显大于中位数时，提醒按中位设而不是按最大那幅

只读不写，不动任何数据。想直接采用某档建议：

```bash
python scripts/tools/probe_resolution.py --apply 1
```
...
**默认 `crop.color: auto` 会自动判断，但你必须知道它在做什么** ——
因为这里搞错**会不可逆地丢信息**：

| 你的图 | 该用什么 | 搞错会怎样 |
|---|---|---|
| 地形图、黑白扫描件 | `grayscale`（auto 会自动选） | — |
| **交通图、水系图、分级着色图** | **`color`（auto 会自动选）** | 压成灰度 → **颜色永久丢失** |

**不同图种的墨迹密度差很多，同一个阈值并不通用：**

| 图种 | 行墨迹占比 | 合适的 `density_threshold` |
|---|---|---|
| 五万分一地形图（密集等高线） | 20~30% | 0.10 |
| 全国交通图（只有道路水系） | **2~9%** | **0.05** |

用错的表现是：内容边界乱跳（可能只框住一小块、或者整幅留白），
`crop_report.json` 里的 `ratios` 明显偏离 1。

**换图种时，先用探测工具量一遍**（`probe_resolution.py`），
它会报出实际尺寸分布和比例异常，比盲试快得多。
...
├── 自定义尺寸裁切.bat           只裁切，运行时可自己填输出宽高
├── 拼图工作台.bat               没有图号时，拖拽确定图幅位置关系
...
│   ├── 原理与踩坑.md
│   ├── 参数调优.md
│   ├── 命名规则与接图表.md        图号换算规则怎么定、怎么用接图表校验
│   ├── 只裁切模式.md              没有规范命名/接图表/经纬度时怎么用
...
└── scripts/
    ...
    └── tools/
        ...
        ├── probe_resolution.py    探测扫描件尺寸，推荐输出分辨率
        ├── make_puzzle.py         生成拼图工作台（拖拽定位置关系）
        └── assemble_by_layout.py  按布局把图幅拼成整幅
```
